...
export default function ExposureTestingPage() {
  ...
  const createTest = async () => {
    ...
    try {
      ...
      setSavedMsg('✅ Test started!'); setTimeout(() => setSavedMsg(''), 2000);
  ...
  const saveResults = async () => {
    ...
    try {
      ...
      setSavedMsg('✅ Results saved!'); setTimeout(() => setSavedMsg(''), 2000);
      setResults(''); setReactions(''); setSelTestId('');
    } catch (e) {
      console.error('Failed to update ExposureTest:', e);
      setSavedMsg('❌ Update failed'); setTimeout(() => setSavedMsg(''), 3000);
    }
  };
    ...
    <div className="page-container">
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 20, borderBottom: '2px solid #E9EDEF' }}>
        {(['new', 'results', 'history'] as const).map(t => (
          <button key={t} onClick={() => setActiveTab(t)} style={{ padding: '10px 16px', border: 'none', background: 'none', cursor: 'pointer', fontWeight: 600, fontSize: 13, color: activeTab === t ? '#4A7BA7' : '#667781', borderBottom: activeTab === t ? '2px solid #4A7BA7' : '2px solid transparent', marginBottom: -2 }}>
      ...
      {activeTab === 'new' && (
        <div>
          <h3 style={{ color: '#DC2626', marginBottom: 16 }}>⚠️ Safety Checklist</h3>
          {SAFETY_CHECKS.map(c => (
      ...
      {activeTab === 'history' && (
        <div>
          {!loaded ? <div style={{ textAlign: 'center', padding: 40, color: '#4A7BA7' }}>Loading...</div>
            : tests.length === 0 ? <div style={{ textAlign: 'center', padding: 40, color: '#ccc' }}><div style={{ fontSize: 40 }}>🧫</div>No tests yet</div>
              ...
              <div key={t.id} style={{ border: '1px solid #E9EDEF', borderRadius: 8, padding: 14, marginBottom: 10, background: '#fff' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <div>
                    ...
                    <div style={{ fontSize: 13, color: '#667781', marginTop: 4 }}>🧪 {t.allergen} · {t.amount}{t.unit} · {t.testDate}</div>
                    {t.results && <div style={{ fontSize: 13, padding: 8, background: '#F0F2F5', borderRadius: 6, marginTop: 6 }}>{t.results}</div>}
                    {t.reactions && <div style={{ fontSize: 13, padding: 8, background: '#FFF5F5', borderRadius: 6, marginTop: 4, color: '#DC2626' }}>⚠️ {t.reactions}</div>}
                  </div>
                  <button onClick={() => deleteTest(t.id)} style={{ background: 'none', border: '1px solid #E9EDEF', borderRadius: 4, width: 26, height: 26, cursor: 'pointer', color: '#DC2626', fontSize: 11, flexShrink: 0 }}>✕</button>
                </div>
              </div>
            ))}
        </div>
